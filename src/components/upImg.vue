  ...
  <div class="home">
    ...
    <div class="photo" v-show="hasPhoto===1">
      ...
      <div class="picRight">
        ...
        <div class="bottom bottom2" v-show="baseUrl!=''&&language==='en'">
          <p class="text" :class="result=='Positive'?'red':'green'">Tumor Detection：<span>{{result}}</span></p>
          <p class="text green" style="margin-top:10px" v-if="result=='Positive' && score=='LOW'">Prognosis Prediction：<span>{{score}} RISK</span></p>
          <p class="text red" style="margin-top:10px" v-if="result=='Positive' && score=='HIGH'">Prognosis Prediction：<span>{{score}} RISK</span></p>
        </div>
        <div class="bottom bottom2" v-show="baseUrl!=''&&language==='cn'">
          <p class="text red" v-if="result=='Positive'" >分析结果：<span>阳性</span></p>
          <p class="text green" v-if="result=='Negative'">分析结果：<span>阴性</span></p>
          <p class="text green" style="margin-top:10px" v-if="result=='Positive'&&score=='LOW'">预后风险等级：<span>低风险</span></p>
          <p class="text red" style="margin-top:10px" v-if="result=='Positive'&&score=='HIGH'">预后风险等级：<span>高风险</span></p>
        </div>
      </div>
    </div>
  </div>
